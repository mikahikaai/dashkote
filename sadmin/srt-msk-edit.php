...
if (isset($_GET['id'])) {
    $id_srt_msk = $_GET['id'];
    $query_srt_msk = "SELECT * FROM tbl_srt_msk m INNER JOIN tbl_bagian b ON b.id_bagian = m.id_bagian WHERE id_srt_msk = $id_srt_msk";
    $result_srt_msk = mysqli_query($conn, $query_srt_msk);
    $row_srt_msk = mysqli_fetch_assoc($result_srt_msk);
    // var_dump($row_srt_msk);
    // die();
}
...
if (isset($_POST['submit'])) {
    ...
    dari = '$dari',
    id_bagian = '$id_bagian',
    file = '$file' WHERE id_srt_msk = $id_srt_msk";
                        ...
                        <div class="card border-top border-0 border-4 border-primary">
                            <div class="card-body px-5 pb-5">
                                ...
                                <form class="row g-3" method="POST" target="" enctype="multipart/form-data">
                                    <input type="hidden" name="fileLama" value="<?= $row_srt_msk['file'] ?>">
                                    <div class="col-12">
                                        <label for="no_srt_msk" class="form-label">Nomor Surat :</label>
                                        <input type="text" class="form-control" name="no_srt_msk" id="no_srt_msk" placeholder="Nomor Surat Masuk" value="<?= $row_srt_msk['no_srt'] ?>" required>
                                    </div>
                                    <div class="col-12">
                                        <label for="tgl_srt" class="form-label">Tanggal Surat :</label>
                                        <input type="date" class="form-control" name="tgl_srt" id="tgl_srt" placeholder="Tanggal Surat Masuk" value="<?= $row_srt_msk['tgl_srt'] ?>" required>
                                    </div>
                                    <div class="col-12">
                                        <label for="lampiran" class="form-label">Lampiran :</label>
                                        <input type="text" class="form-control" name="lampiran" id="lampiran" placeholder="Lampiran Surat Masuk" value="<?= $row_srt_msk['lampiran'] ?>" required>
                                    </div>
                                    <div class="col-12">
                                        <label for="hal" class="form-label">Hal :</label>
                                        <input type="text" class="form-control" name="hal" id="hal" placeholder="Perihal Surat Masuk" value="<?= $row_srt_msk['hal'] ?>" required>
                                    </div>
                                    <div class="col-12">
                                        <label for="dari" class="form-label">dari :</label>
                                        <input type="text" class="form-control" name="dari" id="dari" placeholder="Surat Masuk Dari" value="<?= $row_srt_msk['dari'] ?>" required>
                                    </div>
                                    <div class="col-12">
                                        ...
                                        <select name="id_bagian" id="id_bagian" class="form-select" required>
                                            ...
                                            ?>
                                                <option <?= $row_bagian['id_bagian'] == $row_srt_msk['id_bagian'] ? 'selected' : '' ?> value="<?= $row_bagian['id_bagian']; ?>"><?= $row_bagian['nm_bagian'] ?></option>
                                            <?php } ?>
                                        </select>
                                    </div>
                                    <div class="col-12">
                                        <label for="file_lama" class="form-label">File Lama :</label>
                                        <a href="../assets/files/<?= $row_srt_msk['file'] ?>" target="_blank"><i class="lni lni-link"></i> <?= $row_srt_msk['file'] ?></a>
                                    </div>
                                    <div class="col-12">
